...
public class ScoreServiceV1 {
	 ...
	 * 인스턴스변수들을 private가 아닌 protected로 선언한다
	 * 
	 * protected :
	 * protected는 private의 성질을 가지면서
	 * 상속받은 클래스에서 상속받아 변수를 선언한것과 같은 효과를 낸다
	 * 인스턴스변수도 상속이 된다
	 */
		...
		public int input() {
			...
			for(index = 0; index < students.length ; index++) {
				 ...
				 * 이렇게 되면 for()문이 중단되기 전에 END 무낮열이
				...
				// 학생이름을 배열에 저장할 때는 if() 다음에 위치해야 한다
				// 만약 if() 위에 코드를 위치하면 중단 되기전에
				// student[index] 요소에 END 문자열이 먼저 저장되기 때문에
				// 학생 이름에 END가 나타날 것이다
				students[index] = strName;
